...
run_safe AC-28 'Local records, saved resources, and study coaching round-trip with bounded deterministic state' 'vitest Markdown, resource, study, command, and delivery contracts' npm test -- tests/markdown/codec.test.ts tests/resources tests/study tests/commands
...
run_safe AC-30 'Concurrent backup verifies records, resources, study state, and corruption without plaintext or secrets' 'vitest AC-30 snapshot, resource/study restore, corruption, and plaintext cleanup' npm test -- tests/ops/backup.test.ts -t 'AC-30|consistent online SQLite backup|quarantines staged plaintext|backs up strict resources|resource content tampering|corrupt study schema'
