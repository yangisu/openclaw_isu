...
if [[ "$MODE" == "dry-run" ]]; then
  say "DRY_RUN verify Ubuntu $EXPECTED_UBUNTU, Node >=$MIN_NODE, systemd, lingering"
  say "DRY_RUN verify package-local OpenClaw $EXPECTED_OPENCLAW"
  say "DRY_RUN build, validate, and inspect exactly 5 optional runtime tools"
...
version_ge "$NODE_VERSION" "$MIN_NODE" || { say 'node_version_incompatible'; exit 1; }
...
if [[ "$MODE" == check ]]; then
  [[ -d "$SECRET_DIR" && "$(stat -c %a "$SECRET_DIR")" == 700 ]] || { say 'secret_directory_permissions_invalid'; exit 1; }
  for file in telegram-token naver-caldav naver-oauth; do
    [[ -f "$SECRET_DIR/$file" && "$(stat -c %a "$SECRET_DIR/$file")" == 600 ]] || { say "secret_file_invalid:$file"; exit 1; }
  done
...
node -e 'const x=JSON.parse(process.argv[1]); const tools=x.tools??x.plugin?.tools??[]; if(tools.length!==5||tools.some(t=>t.optional!==true)) process.exit(1)' "$INSPECT_JSON" \
  || { say 'runtime_tool_contract_invalid'; exit 1; }
...
' "$CRON_JSON" "$CRON_KEY" "$CRON_EXPR" "$CRON_MESSAGE" "$OWNER_ID" || { say 'cron_contract_invalid'; exit 1; }
