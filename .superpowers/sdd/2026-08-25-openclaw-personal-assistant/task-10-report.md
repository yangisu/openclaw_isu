...
The installed OpenClaw 2026.7.1 cron CLI/source/types were inspected before choosing the scheduler. Its `payload.kind=command` path parses a JSON string array and passes that array directly to `runCommandWithTimeout`; it does not require a model-backed turn or shell command. Maintenance therefore uses two fixed-argv OpenClaw jobs: daily at `0 3 * * *` and monthly at `0 4 1 * *`, both `Asia/Seoul`, exact (`staggerMs: 0`), isolated, no delivery, no secret environment, and guarded by exact trigger bytes to suppress catch-up. The 03:00/04:00 times are the deterministic ruling for design frequencies that did not specify times.

One production orchestrator now owns daily order: create, exact verify, isolated sample restore, durable health recovery, then retention (30 verified points/minimum 2). Any publication-unknown, identity/media, backup, restore, or health failure reports durable backup health and leaves retention at zero. Monthly selects an exact archive, verifies it, performs the existing full isolated restore verification, and records its schema/hash-bound evidence without live apply. SQLite `BEGIN IMMEDIATE` provides cross-process non-overlap and all repositories, health stores, locks, and restore temporary roots close in finally/verified cleanup paths.
